...
import augmenter
from TransitionProbs import getTransProbs
...
def main(
    ...
    freq_balance=0.6,
    base_restart=0.04,
    #base_time_travel=0.13,
    #adjust_scale=0.8,
    window_size = 3,
    time_travel_scale=0.5,
    eps=1e-3,
    K=100,
    ...
    **kwargs
    ...
    if augment_method == 'STAA' and not symmetric_trick:
    ...
    if augment_method == 'STAA':
        restart_probs, time_travel_probs = getTransProbs(
            dataset, base_restart, device, dense, window_size, freq_balance, time_travel_scale
        )
        STAA_arguments = (
            restart_probs, time_travel_probs,
            eps, K, symmetric_trick, device, dense, verbose
        )
        augment_method = augmenter.STAA(*STAA_arguments)
